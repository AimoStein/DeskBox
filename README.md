...
2. 想把盒子摆整齐，打开**设置窗口**（托盘图标双击，或再运行一次 `JianZhuo.exe`），点最上面的**「一键整理盒子」**；想把桌面上的文件自动归类，就点下面的**「整理桌面文件到盒子」**。
...
JianZhuo.exe --action=arrange      一键整理盒子（排列整齐）
JianZhuo.exe --action=organize     整理桌面文件到盒子
