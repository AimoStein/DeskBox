...
public partial class App : Application
{
    ...
    protected override void OnStartup(StartupEventArgs e)
    {
        ...
        else if (firstRun)
        {
            _tray.ShowInfo(
                "简桌已就绪",
                "在桌面空白处右键 →「简桌」，就能一键整理、新建盒子；双击桌面空白处可以隐藏所有图标。");
        }
        else
        {
            ShowSettings();
    ...
    private void HandleCommand(string command, string? argument)
    {
        switch (command)
        {
            ...
            case "openlog":
                ...
                {
                    System.Windows.MessageBox.Show("暂时还没有日志。", "简桌", MessageBoxButton.OK, MessageBoxImage.Information);
                }
    ...
    private void RunOrganize()
    {
        ...
        if (System.Windows.MessageBox.Show(message, "简桌 · 一键整理", MessageBoxButton.OKCancel, MessageBoxImage.Question)
            != MessageBoxResult.OK)
        {
    ...
    private void RunUndo()
    {
        ...
        _tray?.ShowInfo("简桌", $"已还原 {restored} 项。");
    }
    ...
    private static void ShowAbout()
    {
        ...
        System.Windows.MessageBox.Show(
            ...
            "· 配置位置：%AppData%\\JianZhuo\\config.json",
            "关于简桌",
            MessageBoxButton.OK,
